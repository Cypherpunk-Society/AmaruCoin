...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Amarucoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Amarucoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Amarucoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Amarucoin 0.7 datadir."
    ;;
  3)
    echo "Detected Amarucoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Amarucoin 0.8 datadir."
    ;;
esac
