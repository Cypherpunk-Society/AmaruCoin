...
const char * const BITCOIN_CONF_FILENAME = "amarucoin.conf";
const char * const BITCOIN_PID_FILENAME = "amarucoin.pid";
...
#else
    const char* pszModule = "amarucoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Amarucoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Amarucoin";
#else
    // Unix
    return pathRet / ".amarucoin";
#endif
#endif
}
